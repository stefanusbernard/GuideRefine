...
**1. `02_forge_T2T_bsgenome.Rmd`** — open and run in RStudio (it uses `rstudioapi` to find its own path, so it can't be `Rscript`'d). It downloads the T2T-CHM13v2.0 genome from NCBI (accession `GCF_009914755.1`, ~3 GB) and forges + installs the `BSgenome.Hsapiens.NCBI.T2TCHM13v2.0` package. This package isn't on Bioconductor, so this is the only way to get it.

**2. `01_T2T_gff_to_ccds_conversion.Rmd`** — produces the T2T equivalent of the CCDS file (`T2T-CHM13v2.0_gene_annot_granges.rds`). First download, for accession `GCF_009914755.1`, from the [NCBI Genome page](https://www.ncbi.nlm.nih.gov/datasets/genome/GCF_009914755.1/):
